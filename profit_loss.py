...
contracts = {
    ...
    # Micro FX
    "M6B": {"tick_size": 0.0001, "tick_value": 0.625},
    "M6E": {"tick_size": 0.00005, "tick_value": 0.625},
    "M6J": {"tick_size": 0.0000005, "tick_value": 0.50},
}
...
# Inputs
# -------------------------
contract = st.selectbox("Contract", list(contracts.keys()))

entry = st.number_input("Entry Price", format="%.6f")
stop = st.number_input("Stop-Loss Price", format="%.6f")
target = st.number_input("Take-Profit Price", format="%.6f")

contracts_count = st.number_input("Number of Contracts", min_value=1, value=1)
